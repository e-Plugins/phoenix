...
require_once 'includes/modules/payment/digiwallet/digiwallet.client.php';
...
if (mysqli_num_rows($transaction_query) > 0) {
    ...
    if (mysqli_num_rows($check_query)) {
        ...
        $paymentCode = substr($transaction_info['issuer_id'], 0, 3);
        $iTest = false;//(constant("MODULE_PAYMENT_DIGIWALLET" . $pay_type . "TESTACCOUNT") == "True") ? 1 : 0;
        $digiCore = new \Digiwallet\ClientCore($transaction_info['rtlo'], $paymentCode, 'nl');
        ...
        if ($isTestMode) { // Always OK
            ...
            $apiToken = constant("MODULE_PAYMENT_DIGIWALLET_{$paymentCode}_DIGIWALLET_API_TOKEN");
            $paidStatus = $digiCore->checkTransaction($apiToken, $transaction_info['transaction_id']);
        }
        ...
        if ($paidStatus) {
            ...
            $sql_data_array = array(
                ...
                'date_added' => 'now()',
                'customer_notified' => '0',
                'comments' => 'Digiwallet result: ' . ($testMode ? "Success (Test mode)" : '') . ($paymentIsPartial ? " - Partially paid" : " - Fully paid")
            );
            $GLOBALS['db']->perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);
            ...
            $email_order = STORE_NAME . "\n" . EMAIL_SEPARATOR . "\n" . EMAIL_TEXT_ORDER_NUMBER . ' ' . $order_id . "\n" . EMAIL_TEXT_INVOICE_URL . ' ' . digiwalletClient::formatOscommerceUrl(tep_site_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . $order_id, 'SSL', false)) . "\n" . EMAIL_TEXT_DATE_ORDERED . ' ' . strftime(DATE_FORMAT_LONG) . "\n\n";
            ...
            if($cancel){
                ...
                Href::redirect(digiwalletClient::formatOscommerceUrl(tep_site_href_link(FILENAME_CHECKOUT_SUCCESS, '', 'SSL', true, false)));
                exit(0);
            } else if($finished){
                // Clear Cart here
                $cart->remove_all();
            ...
            $sql_data_array = array(
                ...
                'date_added' => 'now()',
                'customer_notified' => '0',
                'comments' => 'Digiwallet result ' . $digiCore->getErrorMessage() . ' used transaction id : ' . $transaction_info['transaction_id'] . ' | ' . $transaction_info['transaction_id'] . '|' . $paymentCode . '|'
            );
            $GLOBALS['db']->perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);
            // Update Digiwallet
            if($cancel && !$testMode){
...
if($cancel){
    // Redirect to checkout page
    Href::redirect(digiwalletClient::formatOscommerceUrl(tep_site_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false)));
    exit(0);
} else if($finished){
    // Redirect to checkout page
    check_goto_checkout($pay_type, $order_id);
