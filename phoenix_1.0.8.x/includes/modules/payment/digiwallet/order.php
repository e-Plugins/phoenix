...
class order {
    ...
    function query($order_id) {
        ...
        $totals_query = $GLOBALS['db']->query("select title, text from " . TABLE_ORDERS_TOTAL . " where orders_id = '" . (int)$order_id . "' order by sort_order");
        while ($totals = ($totals_query->fetch_assoc())) {
            $this->totals[] = array('title' => $totals['title'],
                'text' => $totals['text']);
        }

        $order_total_query = $GLOBALS['db']->query("select text from " . TABLE_ORDERS_TOTAL . " where orders_id = '" . (int)$order_id . "' and class = 'ot_total'");
        $order_total = ($order_total_query->fetch_assoc());

        $shipping_method_query = $GLOBALS['db']->query("select title from " . TABLE_ORDERS_TOTAL . " where orders_id = '" . (int)$order_id . "' and class = 'ot_shipping'");
        $shipping_method = ($shipping_method_query->fetch_assoc());

        $order_status_query = $GLOBALS['db']->query("select orders_status_name from " . TABLE_ORDERS_STATUS . " where orders_status_id = '" . $order['orders_status'] . "' and language_id = '" . (int)$languages_id . "'");
        ...
        $index = 0;
        $orders_products_query = $GLOBALS['db']->query("select orders_products_id, products_id, products_name, products_model, products_price, products_tax, products_quantity, final_price from " . TABLE_ORDERS_PRODUCTS . " where orders_id = '" . (int)$order_id . "'");
        while ($orders_products = ($orders_products_query->fetch_assoc())) {
            ...
            $subindex = 0;
            $attributes_query = $GLOBALS['db']->query("select products_options, products_options_values, options_values_price, price_prefix from " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " where orders_id = '" . (int)$order_id . "' and orders_products_id = '" . (int)$orders_products['orders_products_id'] . "'");
